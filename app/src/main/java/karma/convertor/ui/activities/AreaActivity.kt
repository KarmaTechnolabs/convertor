...
import karma.convertor.databinding.ActivityAreaBinding
import karma.convertor.listeners.ItemClickListener
import karma.convertor.viewmodel.WeightViewModel
import java.util.*
import kotlin.collections.ArrayList

class AreaActivity: BaseActivity(), View.OnClickListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.weightResponse.observe(this) {
            ...
            if (it.conversionValue != "") {
                unitActivityList.add(UnitActivityModelResponse("Acre", (it.conversionValue.trim().toDouble() *2.47105).toString()))
                unitActivityList.add(UnitActivityModelResponse("sq km", (it.conversionValue.trim().toDouble()*0.01).toString()))
                unitActivityList.add(UnitActivityModelResponse("sq m", (it.conversionValue.trim().toDouble() * 10000).toString()))
                unitActivityList.add(UnitActivityModelResponse("sq yds", (it.conversionValue.trim().toDouble()*11959.9).toString()))
                unitActivityList.add(UnitActivityModelResponse("sq ft", (it.conversionValue.trim().toDouble() * 107639).toString()))
                unitActivityList.add(UnitActivityModelResponse("sq inch", (it.conversionValue.trim().toDouble()*1.55e+7).toString()))
            ...
            callback = object : PickerLayoutManager.OnItemSelectedListener {
                override fun onItemSelected(layoutPosition: Int) {
                    ...
                    if(layoutPosition == 0) {
                        ...
                        viewModel.weightResponse.observe(this@AreaActivity) {
                            ...
                            unitActivityAdapter?.clear()
                            unitActivityAdapter?.setClickListener(this@AreaActivity)
                            binding.unitRecycler.adapter = unitActivityAdapter
                            unitActivityAdapter?.setItems(unitActivityList)
