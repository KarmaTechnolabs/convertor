...
class BloodActivity : BaseActivity(), View.OnClickListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.header.toolbar.text = resources.getString(R.string.blood)
        ...
        viewModel.weightResponse.observe(this) {
            ...
            if (it.conversionValue != "") {
                unitActivityList.add(UnitActivityModelResponse("mg/dL", (it.conversionValue.trim().toDouble() * 18).toString()))
            } else {
                unitActivityList.add(UnitActivityModelResponse("mg/dL", ""))
            }
            ...
            callback = object : PickerLayoutManager.OnItemSelectedListener {
                override fun onItemSelected(layoutPosition: Int) {
                    ...
                    if (layoutPosition == 0) {
                        ...
                        viewModel.weightResponse.observe(this@BloodActivity) {
                            ...
                            unitActivityAdapter?.clear()
                            unitActivityAdapter?.setClickListener(this@BloodActivity)
                            binding.unitRecycler.adapter = unitActivityAdapter
                            unitActivityAdapter?.setItems(unitActivityList)
                        ...
                        viewModel.weightResponse.observe(this@BloodActivity) {
                            ...
                            unitActivityList.clear()
                            if (it.conversionValue != "") {
                                unitActivityList.add(UnitActivityModelResponse("mmol/L", (it.conversionValue.trim().toDouble() * 0.0555).toString()))
                            } else {
                                unitActivityList.add(UnitActivityModelResponse("mmol/L", ""))
                            ...
                            if (it.conversionValue != "") {
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "Hectare",
                                        (it.conversionValue.trim().toDouble() * 100).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "Acre",
                                        (it.conversionValue.trim().toDouble() * 247.105).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq m",
                                        (it.conversionValue.trim().toDouble() * 1e+6).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq yds",
                                        (it.conversionValue.trim().toDouble() * 1.196e+6).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq ft",
                                        (it.conversionValue.trim().toDouble() * 1.076e+7).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq inch",
                                        (it.conversionValue.trim().toDouble() * 1.55e+9).toString()
                                    )
                                )
                        ...
                        viewModel.weightResponse.observe(this@BloodActivity) {
                            ...
                            if (it.conversionValue != "") {
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "Hectare",
                                        (it.conversionValue.trim().toDouble() * 1e-4).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                ...
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq yds",
                                        (it.conversionValue.trim().toDouble() * 1.19599).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq ft",
                                        (it.conversionValue.trim().toDouble() * 10.7639).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq inch",
                                        (it.conversionValue.trim().toDouble() * 1550).toString()
                                    )
                                )
                            ...
                            if (it.conversionValue != "") {
                                ...
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq km",
                                        (it.conversionValue.trim()
                                            .toDouble() * 8.3613e-7).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq m",
                                        (it.conversionValue.trim().toDouble() * 0.836127).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq ft",
                                        (it.conversionValue.trim().toDouble() * 9).toString()
                                    )
                                )
                                unitActivityList.add(
                                    UnitActivityModelResponse(
                                        "sq inch",
                                        (it.conversionValue.trim().toDouble() * 1296).toString()
                                    )
                                )
