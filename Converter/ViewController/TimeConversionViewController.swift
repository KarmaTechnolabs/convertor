//
//  TimeConversionViewController.swift
//  Converter
//
...
class TimeConversionViewController: UIViewController,CustomNumericKeyboardDelegate {

        @IBOutlet weak var viewScroller: UIScrollView!
        @IBOutlet weak var outerStackView: UIStackView!
        @IBOutlet weak var outerStackViewTopConstraint: NSLayoutConstraint!
        @IBOutlet weak var hourTextField: UITextField!
        @IBOutlet weak var hourTextFieldStackView: UIStackView!
        @IBOutlet weak var minuteTextField: UITextField!
        @IBOutlet weak var minuteTextFieldStackView: UIStackView!
        @IBOutlet weak var secondTextField: UITextField!
        @IBOutlet weak var secondTextFieldStackView: UIStackView!
        @IBOutlet weak var milliSecondTextField: UITextField!
        @IBOutlet weak var milliSecondTextFieldStackView: UIStackView!
        @IBOutlet weak var microSecondTextField: UITextField!
        @IBOutlet weak var microSecondTextFieldStackView: UIStackView!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        hourTextField._lightPlaceholderColor(UIColor.lightText)
        hourTextField.setAsNumericKeyboard(delegate: self)
        
        minuteTextField._lightPlaceholderColor(UIColor.lightText)
        minuteTextField.setAsNumericKeyboard(delegate: self)
        
        secondTextField._lightPlaceholderColor(UIColor.lightText)
        secondTextField.setAsNumericKeyboard(delegate: self)
        
        milliSecondTextField._lightPlaceholderColor(UIColor.lightText)
        minuteTextField.setAsNumericKeyboard(delegate: self)
        
        microSecondTextField._lightPlaceholderColor(UIColor.lightText)
        microSecondTextField.setAsNumericKeyboard(delegate: self)
    ...
    @IBAction func handleSaveButtonClick(_ sender: UIBarButtonItem) {
        if !isTextFieldsEmpty() {
                      let conversion = "\(hourTextField.text!) hours = \(minuteTextField.text!) minutes = \(secondTextField.text!) seconds = \(milliSecondTextField.text!) milliseconds = \(microSecondTextField.text!) microseconds"
                      
                      var arr = UserDefaults.standard.array(forKey: TIME_USER_DEFAULTS_KEY) as? [String] ?? []
                      
                      if arr.count >= TIME_USER_DEFAULTS_MAX_COUNT {
                          arr = Array(arr.suffix(TIME_USER_DEFAULTS_MAX_COUNT - 1))
                      }
                      arr.append(conversion)
                      UserDefaults.standard.set(arr, forKey: TIME_USER_DEFAULTS_KEY)
    ...
    func isTextFieldsEmpty() -> Bool {
              if !(hourTextField.text?.isEmpty)! && !(minuteTextField.text?.isEmpty)! &&
                  !(secondTextField.text?.isEmpty)! && !(milliSecondTextField.text?.isEmpty)! &&
                  !(microSecondTextField.text?.isEmpty)! {
                  return false
              }
              return true
          }
    ...
    func updateTextFields(textField: UITextField, unit: TimeUnit) -> Void {
        if let input = textField.text {
            if input.isEmpty {
                ...
                let time = Time(unit: unit, value: value)
                
                for _unit in TimeUnit.getAllUnits {
                    if _unit == unit {
                        continue
                    }
                    let textField = mapUnitToTextField(unit: _unit)
                    let result = time.convert(unit: _unit)
    ...
    func mapUnitToTextField(unit: TimeUnit) -> UITextField {
        ...
        case .millisecond:
            textField = milliSecondTextField
        case .microsecond:
            textField = microSecondTextField
        }
        return textField!
    }
    ...
    func clearTextFields() {
        ...
        milliSecondTextField.text = ""
        microSecondTextField.text = ""
    }
